...
def set_link_lengths(New_L1=None,New_L2=None,EOM = None):
	"""
...
def animate_plots(response,ReachType, Weighted=False, save_as_gif = False):
	...
	if response == True:
		...
		global A1,A2,L1,L2
		A1_forward = np.array(list(reversed(A1.T))).T
		A2_forward = np.array(list(reversed(A2.T))).T
		A1_reverse = A1
		A2_reverse = A2

		if Weighted == True:
			Vm_forward = WeightedNormalizedMuscleVelocity_Forward
			Vm_reverse = WeightedNormalizedMuscleVelocity_Reverse
		else:
			Vm_forward = NormalizedMuscleVelocity_Forward
			Vm_reverse = NormalizedMuscleVelocity_Reverse
...
EOM = 'Uno'
	...
	# Set Link Lengths and Define Moment Arm Matrix Functions for the model and establish flexor/extensor list
