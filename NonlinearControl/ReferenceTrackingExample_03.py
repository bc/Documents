...
def return_MA_matrix_functions(AllMuscleSettings):
    ...
    ṘT_symbolic = sp.Matrix(sp.diff(RT_symbolic,θ_EFE))
    # RT_func = lambdify([θ_SFE,x1,θ_PS],RT_symbolic)
    # ṘT_func = lambdify([θ_SFE,x1,θ_PS],ṘT_symbolic)
    ...
    return(RT_symbolic,ṘT_symbolic)

t = sy.Symbol('t')

x1 = sy.Function('x1')(t) # θ
x2 = sy.Function('x2')(t) # ω
x3 = sy.Function('x3')(t) # f1
x4 = sy.Function('x4')(t) # f2
x5 = sy.Function('x5')(t) # lm1
x6 = sy.Function('x6')(t) # lm2
x7 = sy.Function('x7')(t) # vm1
x8 = sy.Function('x8')(t) # vm2
u1 = sy.Function('u1')(t) # α1
u2 = sy.Function('u2')(t) # α2

g,l = 9.80, 0.45 #m/s², m
M = 3 # kg
α1 = 10*np.pi/180 # rads
α2 = 10*np.pi/180 # rads
m1 = 1 # kg
m2 = 1 # kg
bm1 = 1 # kg/s
bm2 = 1 # kg/s
AllMuscleSettings = return_muscle_settings()
[[r1,r2],[dr1,dr2]]= return_MA_matrix_functions(AllMuscleSettings)
cT = 27.8
kT = 0.0047
LrT = 0.964
β = 1.55
ω = 0.75
ρ = 2.12
PCSA1 = 0.03**2*np.pi # cm²
PCSA2 = 0.03**2*np.pi # cm²
lo1 = 0.10 # m
lo2 = 0.10 # m
c_1 = 23.0
k_1 = 0.046
...
g3 = cT*(1-sy.exp(-x3/(cT*kT)))
g4 = cT*(1-sy.exp(-x4/(cT*kT)))
g5 = sy.sign(-dr1.subs([(θ_EFE,x1),(θ_PS,np.pi/2)])*x2)\
                *x2*sy.sqrt(dr1.subs([(θ_EFE,x1),(θ_PS,np.pi/2)])**2 \
                    + r1.subs([(θ_EFE,x1),(θ_PS,np.pi/2)])**2) # Should be g4
g6 = sy.sign(-dr2.subs([(θ_EFE,x1),(θ_PS,np.pi/2)])*x2)\
                *x2*sy.sqrt(dr2.subs([(θ_EFE,x1),(θ_PS,np.pi/2)])**2 \
                    + r2.subs([(θ_EFE,x1),(θ_PS,np.pi/2)])**2) # Should be g5
g7 = 35*PCSA1*sy.exp(-abs(((x5/lo1)**β-1)/ω)**ρ) # Should be g6
g8 = c_1*k_1*sy.log(sy.exp(((x5/lo1)-Lr1)/k_1) + 1) + η*(x7/lo1) # Should be g7
g9 = c_1*k_1*sy.log(sy.exp(((x6/lo2)-Lr1)/k_1) + 1) + η*(x8/lo2)
g10 = 35*PCSA2*sy.exp(-abs(((x6/lo2)**β-1)/ω)**ρ) # Should be g8

dx1 = x2
dx2 = c1*sy.cos(x1) + c2*g1*x3 - c2*g2*x4
dx3 = g3*g5 - c3*g3*x7
dx4 = g4*g6 - c4*g4*x8
dx5 = x7
dx6 = x8
dx7 = c5*x3 - c6*g7*u1 - c6*g8 - c7*x7 + c8*x7**2/x5
dx8 = c9*x4 - c10*g10*u2 - c10*g9 - c11*x8 + c12*x8**2/x6
