...
def calculate_potential_variability(X,δT1,δT2,dt=0.001,EOM = 'Uno',scheme = "Total"):
	...
	n_shoulder = sum([not el.all() for el in (δT1 == 0)])
	n_elbow = sum([not el.all() for el in (δT2 == 0)])
	δT = (np.concatenate((δT1/n_shoulder,δT2/n_elbow),axis=1)*np.sign(MomentArmMatrix.T)).T
...
def global_R_matrix():
	...
	DELTp_Coefficients = {\
		'Shoulder' : {\
			'MA' : Pigeon_coeff_conversion([-23.8165173, -4.486164 ,  5.8655808,  6.5003255, -8.2736695,2.0812998]), \
			'src' : 'Pigeon', 'eq' : None, 'threshold' : None, \
			'dof' : 'Shoulder'}, \
		'Elbow' : {\
			'MA' : 0,\
...
def calculate_weighted_unscaled_potential_torque_variations_REACHING_TASK():
	...
	"""
	# Uncomment to remove afferented scaling.
	CorrectedAfferentNumber = np.array([100 for key in AllCoefficients.keys()],dtype='float64')[:,np.newaxis] # returns an (m,1) array
