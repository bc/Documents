...
def find_X_values(t,Xi,Xf):
	"""
	SINE WAVE IMPLEMENTATION. USE TIME ARRAY LONGER THAN 2.
	...
	x = np.array(list(map(lambda t: (xi+xf)/2 + ((xf-xi)/2)*sin(pi*t - pi/2),t.T)),\
					dtype='float128',ndmin=2) # Returns an (1,N) array
	y = np.array(list(map(lambda t: (yi+yf)/2 + ((yf-yi)/2)*sin(pi*t - pi/2),t.T)),\
					dtype='float128',ndmin=2) # Returns an (1,N) array
	X = np.concatenate((x,y),axis=0) # Returns an (2,N) array

		# Test of singularities
	assert (((X**2).sum(axis=0)**0.5)<L1+L2).all(), "Trajectory creates singularities."
...
def find_Ẋ_values(t,Xi,Xf,t_end = 1):
	...
	if np.shape(Xi) == (2,1): xi,yi = xi[0],yi[0]
	if np.shape(Xf) == (2,1): xf,yf = xf[0],yf[0]
	ẋ = np.array(list(map(lambda t: pi*((xf-xi)/2)*cos(pi*t - pi/2),t.T)), dtype = 'float128', ndmin=2) # Returns an (1,N) array
	ẏ = np.array(list(map(lambda t: pi*((yf-yi)/2)*cos(pi*t - pi/2),t.T)), dtype = 'float128', ndmin=2) # Returns an (1,N) array
...
def find_Ẍ_values(t,Xi,Xf,t_end=1):
	...
	if np.shape(Xi) == (2,1): xi,yi = xi[0],yi[0]
	if np.shape(Xf) == (2,1): xf,yf = xf[0],yf[0]
	ẍ = np.array(list(map(lambda t: -(pi**2)*((xf-xi)/2)*sin(pi*t - pi/2),t.T)), dtype = 'float128', ndmin=2) # Returns an (1,N) array
	ÿ = np.array(list(map(lambda t: -(pi**2)*((yf-yi)/2)*sin(pi*t - pi/2),t.T)), dtype = 'float128', ndmin=2) # Returns an (1,N) array
...
def calculate_potential_variability(X,δT1,δT2,dt=0.001,EOM = 'Uno',scheme = "Total"):
	...
	assert (abs(np.array([np.matrix(M_inv[i])*(T[i]-CȦ[i]-BȦ[i]) for i in range(A1.size)]).T -Ä.T)<1e-12).all(), "There is an issue with the mapping from Torques to Angular Accelerations."
...
def reaching_task(dt = 0.001, Xi = [0,0.25], Xf = [0.25,0.50],t_end=1):
	...
	set_link_lengths()
	create_angle_lists()
	t = np.array(np.arange(0,2+dt,dt),dtype='float128',ndmin=2)
...
if SaveOutputFigures == True:
	pdf_sinusoid = PdfPages(DescriptiveTitle.replace(' ','_') + '_Sinusoid.pdf')
	pdf_compare = PdfPages(DescriptiveTitle.replace(' ','_') + '_Compare_Sinusoid.pdf')
...
t,X = reaching_task(Xi=Xi, Xf=Xf,dt=dt, t_end=t_end)
return_MA_matrix()

	# Calculate resulting muscle velocities, lengths, etc.

NormalizedMuscleVelocity = calculate_muscle_velocities()
WeightedPotentialTorqueVariation_shoulder,\
WeightedPotentialTorqueVariation_elbow = \
calculate_weighted_unscaled_potential_torque_variations_REACHING_TASK()
WeightedNormalizedMuscleVelocity = calculate_weighted_muscle_velocities_REACHING_TASK()
...
EccentricTorqueVariations_Shoulder = \
	ScalingFactor*eccentric_velocities(WeightedPotentialTorqueVariation_shoulder)
EccentricTorqueVariations_Elbow =\
 	ScalingFactor*eccentric_velocities(WeightedPotentialTorqueVariation_elbow)

PotentialVariability = calculate_potential_variability(X,\
									EccentricTorqueVariations_Shoulder,\
										EccentricTorqueVariations_Elbow,\
											dt=dt,EOM = 'Uno',scheme = "Total")

EccentricCost_Forward = PotentialVariability[0,:int((t.size-1)/2)].mean()
EccentricCost_Reverse = PotentialVariability[0,int((t.size-1)/2):].mean()

	# Plot Afferent-Weighted Muscle Velocity (Forward)

fig1e = plt.figure()
[plt.plot(t.T,WeightedNormalizedMuscleVelocity[i].T) for i in OrderNumber]
ax1e = plt.gca()
ax1e.set_xlim(0,2*(1.3))
ax1e.set_ylim(-12,12)
if t_end!=1:
...
fig1a = plt.figure()
[plt.plot(t.T,NormalizedMuscleVelocity[i].T) for i in OrderNumber]
ax1a = plt.gca()
ax1a.set_xlim(0,2*(1.3))
...
ax4.set_ylim(0,0.01)
ax4.set_yticks([0,0.005,0.01])
ax4.set_yticklabels(['0','','0.01'])
...
percent_cycle = np.concatenate((\
				((((X-X[:,0][:,np.newaxis])**2).sum(axis=0)**0.5)/0.35)[:int((t.size-1)/2)],\
					2-((((X-X[:,0][:,np.newaxis])**2).sum(axis=0)**0.5)/0.35)[int((t.size-1)/2):]),\
						axis=0)
fig3 = plt.figure()
plt.plot(t.T,PotentialVariability.T,'k',lw=2)
...
if SaveOutputFigures == True:
	pdf_sinusoid.savefig(fig1e)
	pdf_sinusoid.savefig(fig1a)
	pdf_sinusoid.savefig(fig1b)
	pdf_sinusoid.savefig(fig1c)
	# pdf_sinusoid.savefig(fig1d)
