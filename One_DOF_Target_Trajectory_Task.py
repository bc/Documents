...
def one_dof_target_trajectory_task(KinematicArrays, Time, **kwargs):
	"""
	# One DOF Target Trajectory Task
	# with Four Muscles
	# August 2016 - USC
	# Daniel A Hagen
	...
	**kwargs
	...
	for i in range(np.shape(MomentArms)[0]):
		...
		ax = fig1.add_subplot(np.shape(MomentArms)[0]*100 + 10 + i+1)
		ax.plot(Time,a_star)
		ax.set_ylabel('Muscle Activation')
		ax.legend(['Muscle 1','Muscle 2','Muscle 3','Muscle 4'])
		if i == 0:
			ax.set_title("F_max = [{}, {}, {}, {}]\nR = [{}, {}, {}, {}]"
		...
		if plotTorques == True:
			Torques = np.concatenate([MomentArms[i,:]*maximum_muscle_force(OptimalLengths, Angle[j], \
						AngularVelocity[j], MomentArms[i,:], OptimalForces)*(a_star[j]*np.identity(4))	for j in range(len(Time))], axis = 0)
			AX = fig2.add_subplot(np.shape(MomentArms)[0]*100 + 10 + i+1)
			AX.plot(Time, Torques)
			AX.set_ylabel('Torque from Each Muscle')
			AX.legend(['Muscle 1','Muscle 2','Muscle 3','Muscle 4'])
			if i == 0:
				AX.set_title("F_max = [{}, {}, {}, {}]\nR = [{}, {}, {}, {}]"
						.format(OptimalForces[0,0], OptimalForces[0,1], OptimalForces[0,2], OptimalForces[0,3],\
				...
				AX.set_title("R = [{}, {}, {}, {}]"
						.format(MomentArms[i,0],MomentArms[i,1],MomentArms[i,2],MomentArms[i,3]))
	if plotTorques == True: AX.set_xlabel('Time (s)')
	...
	if plotFLV == True:
		...
		ax5.plot_surface(L, V, F, rstride=1, cstride=1, cmap=cm.coolwarm,
